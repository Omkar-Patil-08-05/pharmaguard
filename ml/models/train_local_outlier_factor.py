...
from ml.utils.config import (
    LOCAL_OUTLIER_FACTOR_MODEL,
    LOCAL_OUTLIER_FACTOR_RESULTS,
    LOCAL_OUTLIER_FACTOR_SUMMARY
)

logger = setup_logger("LocalOutlierFactor")
...
def train():
    ...
    logger.info("Initializing Local Outlier Factor")
...
def generate_risk_score(scores):
    """
    Convert Local Outlier Factor decision scores
...
def save_results(
    ...
    with open(
        ...
        report.write("# Local Outlier Factor Report\n\n")
...
def main():
    ...
    save_model(
        model,
        LOCAL_OUTLIER_FACTOR_MODEL.name
    )
